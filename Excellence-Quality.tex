...
%In the context of security games  of randomly selecting passengers to be searched is inherently unpredictable). 
...
ML has proposed methods and techniques that borrow from the fields of statistics (in order to take into account the uncertainty about the data) and optimisation (so as to create fast converging methods, minimising for instance, the number of actions required to achieve a specific level of performance). %It has proven to make a difference in practice and has nice mathematical background. We want to focus here on two particular methods that are interesting in machine learning and where Victor and David are experts.
