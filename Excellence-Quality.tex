...
on the road network throughout the city. This problem has not been studied in its repeated form, where a pursuit-evasion games is played multiple times against a series of different smugglers. Therefore the strategy of the defender is not adaptive to the previous observations. It brings together two interesting work. First the work on combinatorial bandits which is somehow more general and therefore does not capture all the subtility of this precise problem and is suboptimal as it does not restrict the second player to play a path in the graph. Second it is a generalisation of the work of Balcan\cite{Balcan15CR} to the case where the number of $k$ unknown matrix is extremely large but posses a combinatorial structure as each matrix correspond to a path in a graph.  It can be seen as
...
In some applications where a newly created security systems is both not provided with any  historical data and cannot be tested before being use in production, then the learning must be performed online. In this contact it is of high importance that the learning is done as fast as possible. This means that only providing an analysis that shows the asymptotic convergence is definitely not enough. this calls for algorithms that are provided with their finite time sample analysis.

The \textit{cumulative regret} setting is the standard formulation for multi-armed bandits. In this formulation, the objective for the forecaster is to minimize the expected cumulative regret after $n$ pulls $R(n)$ defined as
%
\begin{equation*}
...
Our goal here will be to provide a regret analysis for the online stochastic repeated security games. The first step will be study the how to design a new algorithm borrowing ideas from the UCRL algorithm \cite{Ortener} that was extending the bandits ideas of UCB and analysis to general reinforcement learning problems
For this project collaboration with  Bruno Scherrer, a fellow's coauthor, from Inria Nancy is envisageable as Bruno has recently analyse how reinforcement learning asymptotic analysis could be applied to zero sum games.

Finally an important other additional requirement is that we learn security strategy that are not only of good quality in average but also whose performance is not subject to too much variance when used. This requirement has been well-studied in the statistical community has a risk-averse requirement. Recently it has been extended to bandit analysis  (Cite the work of Amir Sani) this calls for an implementation in the security games specific context.
