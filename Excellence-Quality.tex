...
%
%This is close but more general than the recent work of Goldberg et. al.\cite{goldberg2014query}, who assume that the probing outcome corresponds to the {\em true value} of $\boldsymbol A_{i,j}$. It is important to note that unlike the setting that we propose, the simplified version previously considered does not require the analysis of the number of queries required to correctly estimate each entry of the utility matrix, which in turn depends on the performance of the strategies that would use this enrty. 
